...
def generate_and_submit_job(job_config_path, output_dir="/home/tus53997/Benchmark_DNACompression/jobs"):
    job_name = os.path.splitext(os.path.basename(job_config_path))[0] + "_job"  # e.g., sz3_job
...
# Execute Python script using Singularity container
singularity exec --bind /home/tus53997:/mnt sz3_perf_amd.sif python3 Executer.py -c {job_config_path}
"""
    ...
    try:
        with open(bench_config_path, 'r') as file:
            ...
            for job_file in job_files:
                job_config_path = os.path.join(os.path.dirname(bench_config_path), job_file)
                print(f"Processing job configuration: {job_file}")
                generate_and_submit_job(job_config_path)
...
def main():
    ...
    try:
        with open(bench_config_path, 'r') as file:
            bench_config = json.load(file)
            print("Json loaded: ", bench_config)
            job_files = bench_config.get("jobs", [])
            print("Job Files: ", job_files)
            for job_file in job_files:
                print(f"Processing job configuration: {job_file}")
